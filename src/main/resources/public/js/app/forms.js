(function() {
  ...
  function updateStateOnFieldChange(currentState, fieldName, fieldValue) {
    ...
    return {
      ...
      values: update(currentState.values, {
        $merge: values
      })
    };
  }
      ...
      return (
        <Modal show={this.props.open} onHide={this.props.onClose} animation={false}>
          <Modal.Header className="bg-info" closeButton>
            <Modal.Title className="text-primary">Add New Owners</Modal.Title>
          </Modal.Header>
          ...
          <form name="ownersForm" role="form" onSubmit={this.handleSubmit}>
            <Modal.Body className="bg-warning">
              <FormGroup className="text-primary" name="owners" label="Owners (*)" hint="Enter owner(s) separated by comma"
  ...
  window.App.Components.CompanyForm = React.createClass({
    ...
    fetchCompany: function(url) {
      $.ajax({
        ...
        success: function(data) {
          var values = update(data, {
            owners: {
              $set: data.owners.join(', ')
            }
          });
          this.setState({
            values: values
    ...
    render: function() {
      return (
    	 ...
    	 <Modal show={this.props.open} onHide={this.props.onClose} animation={false}>
          <Modal.Header className="bg-info" closeButton>
            <Modal.Title className="text-primary">Enter Company Info</Modal.Title>
          </Modal.Header>
